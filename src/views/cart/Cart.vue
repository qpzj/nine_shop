        ...
        <div :key="index" class="cart-item van-hairline--bottom" v-for="(item,index) in cartList">
          <div class="cart-item-main">
            ...
            <div @click="linkToDetail(item.id)" class="cart-item-image">
              <img :src="$baseUrl+item.main_img_url" alt="img">
            </div>
...
export default {
  ...
  methods: {
    ...
    addOrder() {
      ...
      if (this.amount > this.tolalPrice) {
        this.$api
          .insertAccount({
            money: -this.tolalPrice
          })
          .then(res => {
            this.$api
              .addOrderList({
                order_no: user_id + timestamp,
                user_id: user_id,
                total_price: this.tolalPrice,
                  ...
                  .then(res => {
                    if (res.data.code == 0) {
                      ...
                      const timer = setTimeout(() => {
                        ...
                        clearInterval(timer);
                        this.delALLProduct();
                        Dialog({ message: "支付成功！这里可以查看历史订单" });
