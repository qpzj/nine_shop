<template>
  <div>
    ...
    <div class="btn-block">
      <van-button @click="onDelete" round size="large" type="danger">删除</van-button>
    </div>
...
export default {
  ...
  created() {
    ...
    //       this.form.region = this.getAreaName(
  ...
  methods: {
    editContect() {
      let contect = this.$route.query.item;
      if (contect) {
        this.$api.getAddressOne({ id: contect.id }).then(res => {
          const addressInfo = res.data.data;
          this.addressInfo = addressInfo;
          this.form.name = addressInfo.name;
          this.form.phone = addressInfo.mobile;
    ...
    onDelete() {
      if (this.addressInfo.id) {
        this.$api
          .deleteUserAddress({ id: this.addressInfo.id })
          .then(res => {
            if (res.data.code == 0) {
              ...
              this.$toast(res.data.msg);
              localStorage.setItem("contect", null);
            }
          })
          .catch(err => {
            console.log(err);
